'use strict';
 ...
 // About launch the game
  ...
  if(airplanebattle.game.players.onlobby.length >= airplanebattle.config.game.minPlayers && !airplanebattle.game.toStart) {
    ...
    airplanebattle.chat.broadcast("airplanebattle is going to start in 1 minutes");
...
for(var i=0; i < BRGame.players.length; i++){
    if (BRGame.players[i].airplanebattle.kills >= highscore)
    {
...
jcmp.events.CallRemote('airplanebattle_winner_client_true',highplayerscorename);
jcmp.events.CallRemote('airplanebattle_winner_client_true_all',null);
jcmp.events.CallRemote('airplanebattle_winner_client_name',null,highplayerscorename.escapedNametagName);


for(var i=0; i < BRGame.players.length; i++){
  var player = BRGame.players[i];
  ...
  player.airplanebattle.deaths = 0;
